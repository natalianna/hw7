//I made it so the ball's velocity and starting point were random
//I made it so you couldnt see the ball until the game started
//There is a sound when a point is scored
//The game ends after 5 points are scored
//Theres a special surpise at the GAMEOVER screen
...
var gameoversound, pointsound, osc;  //i couldnt figure out how to add the sound files to my file
var borders = {
	leftx: 0,
	rightx: 0,
	topy: 0,
...
function updateState() {
	if (GAMESTATE == 'START') {
		...
		//  I tried to constrain the paddles but it doesn't seem to be working and I'm not sure why
		if (keypress.w == true) {
			paddle1.y = paddle1.y - 3;
			constrain(paddle1.y, 100, 350);
		} else if (keypress.s == true) {
			paddle1.y = paddle1.y + 3;
		}
		if (keypress.o == true) {
			paddle2.y = paddle2.y - 3;
			constrain(paddle2.y, 100, 350);
...
// "rendering" function, produces the output that the user will get
//... sounds and visuals 
// right now it only does the drawing!  
//
// you may want to update this so that it plays sounds  
// for ball bounces:  hint: i initialized an oscillator 
// (osc) in setup for you. use millis() to log the 
// start time and turn the oscillator back off after a 
// fixed duration of time has passed.   this may require
// some new variables to track the sound timing information  
//  
// may want to use preloaded sounds from mp3 files 
// (which you load into variables gameoversound and  pointsound)
// and play them when the game enters a POINT or GAMEOVER state.  
// as with the ball bounce sound you 
// will want to track some state for the sound file, 
// so that you dont keep starting it while waiting for the spacebar 
// to be pressed
// 
...
function drawStuff() {
	...
	textStyle(BOLD);
	textSize(20);
	text(score.player1, width / 4, 35);
	text(score.player2, 3 * width / 4, 35);
	textStyle(NORMAL);
	textSize(10);
	text('Player 1', width / 4, 50);
	text('Player 2', 3 * width / 4, 50);
	pop();
	...
	stroke(100);
	line(50, 104, 50, 346)
	line(width - 50, 104, width - 50, 346)
	stroke(175);
	line(width / 2, 104, width / 2, 346)
	pop();

	if (GAMESTATE == 'START') {
		textAlign(CENTER);
		textStyle(BOLD);
		text('Press the spacebar to start', width / 2, 70);
	} else if (GAMESTATE == 'PLAY') {
		osc.amp(0, 0.5);
	} else if (GAMESTATE == 'POINT') {
		osc.amp(0.5,0.05 );
		...
		textSize(12);
		textStyle(NORMAL);
		text('Press the spacebar to continue', width / 2, 70);
		...
		textSize(12);
		text('Press the spacebar to play again', width / 2, 70);
	for (var index = 0; index < 100; index = index + 1) {
    noStroke();
		fill(colors[index]);
    ellipse(e[index], r[index], 10);
    e[index] = e[index] + xSpeed[index];
    r[index] = r[index] + ySpeed[index];
	...
	if (score.player1 == 5) {
		textSize(20);
		text('Winner is Player 1', width / 2, 240);
	}
	if (score.player2 == 5) {
		textSize(20);
		text('Winner is Player 2', width / 2, 240);
	}
}

// this initializes the ball and paddle positions at the 
// beginning of a point.  you
// should change this to randomize ball position and velocity 
// each time so the game is more interesting!!
function initializePositions() {
	var choices = [-4, -3.5, -3, -2.5, -2, 2, 2.5, 3, 3.5, 4];
	ball.x = width / 2;
	ball.y = random(borders.topy, borders.bottomy);
	ball.xvelocity = random(choices);
	ball.yvelocity = random(choices);
